<template>
  <div>
    <p class="font-bold text-2xl text-center p-4">FIREBASE</p>
    <button
      type="button"
    ...
    <div v-if="createSendiri" class="p-5">
      <p class="font-bold font-times" style="color:red;">Note! : tambahkan input dengan nama 'id' akan tetapi inputan disabled</p>
    ...
    getData() {
      ...
      if(this.vdata.table.includes('/',0)){
        ...
        // this.vdata.table=leng
         this.$fire.firestore.collectionGroup(`${leng}`).get().then(res=>{
            let arr=[]
            res.forEach(e=>{
          ...
          this.$fire.firestore.collectionGroup(`${this.vdata.table}`).get().then(res=>{
            let arr=[]
            res.forEach(e=>{
                arr.push({id:e.id,...e.data()})
            })
  ...
  methods: {
    ...
    `;
      let methods = `insert(){
          ...
          this.$fire.firestore.collection(\`${this.vdata.table}\`).doc().set(this.vdata).then(res=>{
              console.log('berhasil')
              alert('simpan data berhasil!')
          })
      },update(){
          this.$fire.firestore.collection(\`${this.vdata.table}\`).doc(this.pilih.id).set(this.vdata,{merge:true}).then(res=>{
              alert('update data berhasil!')
          })
      },remove(){
          if(confirm('Apakah yakin menghapus data?')){
              this.$fire.firestore.collection(\`${this.vdata.table}\`).doc(this.pilih.id).delete().then(res=>{
              alert('delete data berhasil!')
              })
          }
      },
       refreshData() {
         let that = this
         this.$fire.firestore.collection(\`${this.vdata.table}\`).onSnapshot(snap=>{
             let arr = []
             snap.forEach(e=>{
                 arr.push({id:e.id,...e.data()})
             })
