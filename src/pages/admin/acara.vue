<template>
    <div class="p-4 bg-white shadow-lg rounded-lg ml-4">
    <!-- FORM -->
    ...
    >
     <div class="card">
        <div class="card-header">balangankab_acara</div>
        <div class="card-body">
      <!-- FORM --> 
          <tr>
          ...
          <tr>
              <td class='text-uppercase text-sm'>judul_</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <div class="sm-form">
                  <input
          ...
          <tr>
              <td class='text-uppercase text-sm'>tempat_acara_</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <div class="sm-form">
                  <input
                    type="text"
                    placeholder="tempat_acara_"
                    id="tempat_acara"
                    name="tempat_acara"
                    class="form-control form-control-sm"
                    v-model="vdata.tempat_acara"
                    required
          ...
          <tr>
              <td class='text-uppercase text-sm'>waktu_acara_</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <div class="sm-form">
                  <input
                    type="text"
                    placeholder="waktu_acara_"
                    id="waktu_acara"
                    name="waktu_acara"
                    class="form-control form-control-sm"
                    v-model="vdata.waktu_acara"
                    required
          ...
          <tr>
              ...
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
              <input type="file" id="filenya" @change="upload">
               <div class="sm-form">
                  <input
                    type="text"
                    placeholder="gambar_"
    ...
    white-space:nowrap"
          >
            ...
            <thead class="mdb-color" :style="{background:thbackground}">
              <tr class="text-dark" style="overflow:scroll;color:dark;">
...
export default {
  ...
  data() {
    return {
      ...
      // valdatable
      datanya: [],
      order:['id','judul','tempat_acara','waktu_acara','tahun','bulan','hari','tgl','tanggal','keterangan','gambar'],
      vdata: {
  ...
  methods: {
    ...
    insert(){
          ...
          let that = this
          let tanggal = this.format(JSON.parse(JSON.stringify(this.vdata.tanggal)));
          let tgl = tanggal.split('-')
          this.vdata.tahun= tgl[3]
          this.vdata.bulan=tgl[2]
          this.vdata.hari=tgl[0]
          this.vdata.tgl = tgl[1]
          db.collection(`balangankab_acara`).doc().set(this.vdata).then(res=>{
        ...
        let that = this
          if(confirm('Apakah yakin menghapus data?')){
              db.collection(`balangankab_acara`).doc(this.vdata.id).delete().then(res=>{
       ...
       refreshData() {
         let that = this
         //doc adalah select *
         db.collection("balangankab_acara").doc().get("*","order by id").then(res => {
          setTimeout(() => {
            (function() {
              var thElm;
              var startOffset;
    ...
    checkedAll() {
     ...
     susun(datas){
      let that=this
      let data = datas;
      let order = ['id','judul','tempat_acara','waktu_acara','tahun','bulan','hari','keterangan','gambar']
      let hasil=[]
      let obj={}
    ...
    // MODAL
    format(date) {
      return this.$datefns.format(date, "dddd-DD-MMMM-YYYY", {
        locale: id
      });
    },
    distanceToNow() {
