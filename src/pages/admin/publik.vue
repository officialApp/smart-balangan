<template>
    ...
    >
     <div class="card">
        <div class="card-header">list_publik</div>
        <div class="card-body">
      <!-- FORM --> 
          <tr>
...
export default {
  ...
  methods: {
    ...
    insert(){
          ...
          let that = this
          db.collection(`list_publik`).doc().set(this.vdata).then(res=>{
        ...
        let that = this
          if(confirm('Apakah yakin menghapus data?')){
              db.collection(`list_publik`).doc(this.vdata.id).delete().then(res=>{
       ...
       refreshData() {
         let that = this
         db.collection(`list_publik`).doc().get('*','').then(res=>{
          setTimeout(() => {
            (function() {
              var thElm;
              var startOffset;
