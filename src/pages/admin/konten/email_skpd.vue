    ...
    >
     <div class="card">
        <div class="card-header">  <button type="button" @click="$router.push('/admin')" class="btn btn-sm btn-dark  mr-4"><img src="https://upload.egov.balangankab.go.id/svg/arrow-left.svg" 
               class="d-inline" style="height:20px;width:20px;filter: brightness(0) invert(1);" alt=""> Back</button> LIST Email SKPD</div>
        <div class="card-body">
      <!-- FORM --> 
          <tr>
          ...
          <tr>
              <td class='text-uppercase text-sm'>email skpd</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <div class="sm-form">
                  <input
                    type="text"
                    placeholder="email skpd"
                    id="nama_menu"
                    name="nama_menu"
                    class="form-control form-control-sm"
...
export default {
  ...
  methods: {
    ...
    insert(){
          ...
          let that = this
          sdb.collection('smartapp_telpon_penting').doc().set(this.vdata).then(res=>{
              console.log(res)
        ...
        sdb.collection('smartapp_telpon_penting').doc().set(this.vdata).then(res=>{
          ...
          if(confirm('Apakah yakin menghapus data?')){
            sdb.collection("smartapp_telpon_penting").doc(this.vdata.id).delete().then(res => {
                console.log(res);
       ...
       refreshData() {
         let that = this
         sdb.collection("smartapp_telpon_penting").doc().select(`select * from smartapp_telpon_penting`).then(arr => {
          setTimeout(() => {
            (function() {
              var thElm;
              var startOffset;
