    ...
    >
     <div class="card">
        <div class="card-header">  <button type="button" @click="$router.push('/admin')" class="btn btn-sm btn-dark  mr-4"><img src="https://upload.egov.balangankab.go.id/svg/arrow-left.svg" 
               class="d-inline" style="height:20px;width:20px;filter: brightness(0) invert(1);" alt=""> Back</button> LIST KODE POS</div>
        <div class="card-body">
      <!-- FORM --> 
          <tr>
...
export default {
  ...
  methods: {
    ...
    insert(){
          ...
          let that = this
          sdb.collection('smartapp_kode_pos').doc().set(this.vdata).then(res=>{
              console.log(res)
        ...
        sdb.collection('smartapp_kode_pos').doc().set(this.vdata).then(res=>{
          ...
          if(confirm('Apakah yakin menghapus data?')){
            sdb.collection("smartapp_kode_pos").doc(this.vdata.id).delete().then(res => {
                console.log(res);
       ...
       refreshData() {
         let that = this
         sdb.collection("smartapp_kode_pos").doc().select(`select * from smartapp_kode_pos`).then(arr => {
          setTimeout(() => {
            (function() {
              var thElm;
              var startOffset;
